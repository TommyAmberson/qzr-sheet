...
- Bonus question correct *before* Q17
...
A quizzer's first error is free if *all* of:
...
When a quizzer answers incorrectly, their team **cannot jump** on the next question — this is a **toss-up**. The toss-up chain carries forward:

- If the toss-up question is also answered incorrectly (by a different team), *that* team is also locked out of the next question.
...
Questions 16–20 have optional A and B sub-columns. These exist to preserve the rule that Q17–20 always have all three teams eligible to jump.

- If the **numbered question** (e.g. Q18) is answered correctly → A and B are not asked.
- If the numbered question is answered incorrectly → the **A** sub-question becomes a toss-up for the other two teams.
- If the **A question** is answered correctly → B is not asked.
- If the **A question** is also answered incorrectly → the **B** sub-question becomes a bonus for the remaining eligible team.
...
- 3rd foul by the **same quizzer**: foul-out, −10 to individual and team. Does **not** stack with the 3rd-team-foul deduction if both happen on the same foul.
- The quizzer **upon whom** a foul is called is ineligible to answer that numbered question, including its A and B sub-parts.

## Overtime

Overtime is only played when two or more teams are tied after regulation (Q1–20). Only tied teams are eligible to answer overtime questions.
...
- Overtime correct answers do **not** count toward individual quizzer stats (no quiz-out, no individual points).
...
Placement is assigned progressively at each overtime checkpoint — teams that fall behind are placed immediately without waiting for remaining tied teams to finish.
