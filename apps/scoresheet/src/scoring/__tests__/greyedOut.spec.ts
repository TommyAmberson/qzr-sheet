...
describe('greyed-out logic', () => {
  it('correct answer greys out all teams on that column', () => {
    ...
    cells[0]![0]![colIdxOf('1')] = C // team 0, quizzer 0, Q1 correct
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('1'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('1'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('1'))).toBe(true)
  })
  ...
  it('correct answer does NOT grey the next question', () => {
    ...
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('2'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('2'))).toBe(false)
    expect(isGreyed(result, 2, colIdxOf('2'))).toBe(false)
  })
  ...
  it('error greys ONLY the erroring team on the next question (toss-up)', () => {
    ...
    expect(isGreyed(result, 0, colIdxOf('2'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('2'))).toBe(false)
    expect(isGreyed(result, 2, colIdxOf('2'))).toBe(false)
    // Q2 is NOT a bonus for anyone
    expect(isBonusFor(result, 1, colIdxOf('2'))).toBe(false)
    expect(isBonusFor(result, 2, colIdxOf('2'))).toBe(false)
  })

  it('two errors in a row from different teams creates a bonus', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('1')] = E // team 0 errors on Q1
    cells[1]![0]![colIdxOf('2')] = E // team 1 errors on Q2 (toss-up)
    const result = computeGreyedOut(cells, columns)
    // Q3: team 0 and team 1 both greyed → bonus for team 2
    expect(isGreyed(result, 0, colIdxOf('3'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('3'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('3'))).toBe(false)
    expect(isBonusFor(result, 2, colIdxOf('3'))).toBe(true)
  })

  it('toss-up answered correctly breaks the chain', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('1')] = E // team 0 errors on Q1
    cells[1]![0]![colIdxOf('2')] = C // team 1 correct on Q2 (toss-up resolved)
    const result = computeGreyedOut(cells, columns)
    // Q3: nobody greyed — chain resolved
    expect(isGreyed(result, 0, colIdxOf('3'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('3'))).toBe(false)
    expect(isGreyed(result, 2, colIdxOf('3'))).toBe(false)
  })
  ...
  it('foul does NOT grey out other teams or next question', () => {
    ...
    expect(isGreyed(result, 0, colIdxOf('1'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('1'))).toBe(false)
    // Q2 not greyed
    expect(isGreyed(result, 0, colIdxOf('2'))).toBe(false)
  })
  ...
  it('correct on base A/B question greys A and B columns', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('16')] = C
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 0, colIdxOf('16B'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16B'))).toBe(true)
  })
  ...
  it('correct on A question greys B column', () => {
    ...
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('17B'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('17B'))).toBe(true)
  })

  it('error on A/B normal → toss-up on A', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17')] = E // team 0 errors on Q17
    const result = computeGreyedOut(cells, columns)
    // Q17A: team 0 greyed (toss-up)
    expect(isGreyed(result, 0, colIdxOf('17A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('17A'))).toBe(false)
  })

  it('error on A → toss-up on B', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17A')] = E
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('17B'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('17B'))).toBe(false)
  })

  it('error on base A/B question does NOT grey out A and B for all teams', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('16')] = E // team 0 errors on Q16
    const result = computeGreyedOut(cells, columns)
    // Q16 is done (greyed for all)
    expect(isGreyed(result, 0, colIdxOf('16'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16'))).toBe(true)
    // Q16A: only team 0 greyed (toss-up), not all teams
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(false)
    expect(isGreyed(result, 2, colIdxOf('16A'))).toBe(false)
    // Q16B: not greyed at all yet
    expect(isGreyed(result, 0, colIdxOf('16B'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('16B'))).toBe(false)
  })

  it('missed bonus does NOT cause toss-up on next question', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17B')] = MB // team 0 misses bonus on Q17B
    const result = computeGreyedOut(cells, columns)
    // Q17B is done (greyed for all)
    expect(isGreyed(result, 0, colIdxOf('17B'))).toBe(true)
    // Q18: nobody greyed — MB doesn't cause toss-up
    expect(isGreyed(result, 0, colIdxOf('18'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('18'))).toBe(false)
    expect(isGreyed(result, 2, colIdxOf('18'))).toBe(false)
  })

  it('bonus answer does NOT cause toss-up on next question', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17B')] = B // team 0 gets bonus on Q17B
    const result = computeGreyedOut(cells, columns)
    // Q18: nobody greyed
    expect(isGreyed(result, 0, colIdxOf('18'))).toBe(false)
    expect(isGreyed(result, 1, colIdxOf('18'))).toBe(false)
  ...
  it('Q16 is toss-up (Q15 error), Q16 error → Q16B is bonus, Q16A bypassed', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('15')] = E // team 0 errors on Q15
    cells[1]![0]![colIdxOf('16')] = E // team 1 errors on Q16 (toss-up)
    const result = computeGreyedOut(cells, columns)
    // Q16A: bypassed (Skipped), greyed for all
    expect(result.colStatuses[colIdxOf('16A')]).toBe(ColStatus.Skipped)
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16A'))).toBe(true)
    // Q16B: team 0 and team 1 tossed → bonus for team 2
    expect(isGreyed(result, 0, colIdxOf('16B'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16B'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16B'))).toBe(false)
    expect(isBonusFor(result, 2, colIdxOf('16B'))).toBe(true)
  })

  it('Q16 is toss-up, Q16 answered correctly → Q16A and Q16B not asked', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('15')] = E // team 0 errors on Q15
    cells[1]![0]![colIdxOf('16')] = C // team 1 correct on Q16 (toss-up resolved)
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 0, colIdxOf('16B'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16B'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16B'))).toBe(true)
  })

  it('Q16 normal error → Q16A toss-up, Q16A error → Q16B bonus', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('16')] = E // team 0 errors on Q16 (3 teams were eligible)
    cells[1]![0]![colIdxOf('16A')] = E // team 1 errors on Q16A toss-up
    const result = computeGreyedOut(cells, columns)
  ...
  it('Q16 is already a bonus (Q14+Q15 error chain) → Q16A and Q16B not asked', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('14')] = E // team 0 errors on Q14
    cells[1]![0]![colIdxOf('15')] = E // team 1 errors on Q15 (toss-up from Q14)
    // Q16: team 0 + team 1 greyed → bonus for team 2
    const result = computeGreyedOut(cells, columns)
    expect(isBonusFor(result, 2, colIdxOf('16'))).toBe(true)
  ...
  it('Q16 bonus answered → Q16A and Q16B greyed', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('14')] = E
    cells[1]![0]![colIdxOf('15')] = E
    cells[2]![0]![colIdxOf('16')] = B // team 2 gets Q16 bonus
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16A'))).toBe(true)
  ...
  it('Q16 bonus missed → Q16A and Q16B greyed', () => {
    ...
    cells[0]![0]![colIdxOf('14')] = E
    cells[1]![0]![colIdxOf('15')] = E
    cells[2]![0]![colIdxOf('16')] = MB // team 2 misses Q16 bonus
    const result = computeGreyedOut(cells, columns)
    expect(isGreyed(result, 0, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 1, colIdxOf('16A'))).toBe(true)
    expect(isGreyed(result, 2, colIdxOf('16A'))).toBe(true)
  ...
  it('colStatuses: A and B are Skipped when base question is correct', () => {
    ...
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('17A')]).toBe(ColStatus.Skipped)
    expect(result.colStatuses[colIdxOf('17B')]).toBe(ColStatus.Skipped)
  })

  it('colStatuses: B is Skipped when A is resolved (correct)', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17')] = E // error on base → toss-up to A
    cells[1]![0]![colIdxOf('17A')] = C // A correct → B skipped
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('17A')]).toBe(ColStatus.Resolved)
    expect(result.colStatuses[colIdxOf('17B')]).toBe(ColStatus.Skipped)
  })

  it('colStatuses: A and B are Skipped when base is bonus (B answer)', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('16')] = B // bonus on base
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('16A')]).toBe(ColStatus.Skipped)
    expect(result.colStatuses[colIdxOf('16B')]).toBe(ColStatus.Skipped)
  })

  it('colStatuses: A and B are Skipped when base is missed bonus (MB)', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('16')] = MB
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('16A')]).toBe(ColStatus.Skipped)
    expect(result.colStatuses[colIdxOf('16B')]).toBe(ColStatus.Skipped)
  })
  ...
  it('colStatuses: B is Skipped when A is bonus/missed-bonus', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('18')] = E // error on base
    cells[1]![0]![colIdxOf('18A')] = MB // missed bonus on A → B skipped
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('18B')]).toBe(ColStatus.Skipped)
  })

  it('colStatuses: A is Pending (not Skipped) when base has error', () => {
    const cells = blankCells()
    cells[0]![0]![colIdxOf('17')] = E // error on base
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('17A')]).toBe(ColStatus.Pending)
    expect(result.colStatuses[colIdxOf('17B')]).toBe(ColStatus.Pending)
  })
  ...
  it('colStatuses: non-AB columns are never Skipped', () => {
    ...
    cells[0]![0]![colIdxOf('1')] = C
    const result = computeGreyedOut(cells, columns)
    expect(result.colStatuses[colIdxOf('2')]).toBe(ColStatus.Pending)
  })
